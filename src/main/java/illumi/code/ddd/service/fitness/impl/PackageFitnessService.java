...
public class PackageFitnessService {
  ...
  private DDDFitness evaluateDomainModule() {
    ...
    fitness.addIssue(module.getPath().contains(structure.getPath() + "domain." + module.getName()),
        DDDIssueType.MINOR,
        String.format(
            "The module '%s' is not a submodule of the module 'domain'.",
            module.getName()));

    fitness.addIssue(containsOnlyDomain(), DDDIssueType.MAJOR,
        String.format(
            "The module '%s' dose not only contains domain artifacts.",
            module.getName()));

    fitness.addIssue(containsAggregateRoot(module), DDDIssueType.CRITICAL,
        String.format(
            "The module '%s' does not contain an Aggregate Root.",
            module.getName()));
  ...
  private DDDFitness evaluateInfrastructureModule() {
    ...
    fitness.addIssue(module.getPath().contains(structure.getPath() + "infrastructure"),
        DDDIssueType.MINOR,
        String.format(
            "The module '%s' is not an infrastructure module.",
            module.getName()));

    fitness.addIssue(containsOnlyInfrastructure(), DDDIssueType.MAJOR,
        String.format(
            "The module '%s' dose not only contains infrastructure artifacts.",
            module.getName()));
  ...
  private DDDFitness evaluateApplicationModule() {
    ...
    fitness.addIssue(module.getPath().contains(structure.getPath() + "application"),
        DDDIssueType.MINOR,
        String.format(
            "The module '%s' is not an application module.",
            module.getName()));

    fitness.addIssue(containsOnlyApplication(), DDDIssueType.MAJOR,
        String.format(
            "The module '%s' dose not only contains application artifacts.",
            module.getName()));
